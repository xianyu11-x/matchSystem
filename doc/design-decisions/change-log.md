...
本页按任务记录代码结构、功能更新和重要决策中的“发生了什么”和“当前影响”。
同一任务的相关变化可合并记录，不按文件或逐提交记流水账；条目应链接权威说明或 ADR。
未提交的变化须明确标注，历史提交记录保持原样；实现细节以对应提交、ADR 和当前源码为准。
...
| 2026-09-06 | 本次提交 | 明确文档分类、同步义务与任务完成条件 | 代码结构、功能和重要决策须同任务记录；见[执行规范](../../AGENTS.md)与[文档维护约定](../README.md#文档维护约定) |

| 2026-09-06 | 本次变更 | [成局等待与调用耗时分离](adr/match-processing-observation.md) | 独立记录成功成局调用实测耗时，Web 支持选定多局聚合与等待/处理 P95；核心隔离不变 |

## 兼容性结论

2026-09-06：独立研究分支新增 [Python Provider 原型与决策](adr/python-provider-research.md)，
证明三类 Provider 可通过宿主进程适配参与模拟器成局，保持核心 Go 不变。覆盖重载、
精度、异常、超时回收与节点隔离，记录调用成本；当前 HTTP/UI 不支持脚本绑定。
