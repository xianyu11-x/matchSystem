...
表达式面板只显示当前节点的操作、直接参数和子输入摘要，不再递归展开整棵子树。
点击“跳转到…”进入对应的下一级表达式；普通表单通过“返回上一级”逐级返回，修改即时
保留在当前规则草稿中。数组子输入逐项提供跳转、上移和删除；Scalar envelope 元数据及
未编辑的兄弟输入保持不变。

高级规则图的节点面板优先跳转到该输入连线的来源节点，并将画布定位到该节点；尚无
连线时进入本地子表达式编辑。跳转本身不修改规则、不新增连线。
