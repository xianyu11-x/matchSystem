...
Windows 桌面端已在真实 Tauri / WebView2 中验证 PNG、SVG、CSV 默认直接写入系统「下载」目录，无需额外原生保存命令或权限。保存按钮不强制弹出「另存为」；页面显示「已交给下载管理器」仅表示发起下载，用户取消或宿主下载失败均不能视为保存成功，应以下载管理器和实际文件为准。验证环境与文件检查见[桌面导出验收](../design-decisions/testing/simulator-enhancements-acceptance.md#2026-09-06-tauriwebview2-桌面导出实测)。
...
比赛分析的“分析范围”可按 RuleKey 筛选，再结合时间窗口与比赛勾选，统计该范围内
已成局成员的属性快照。新增“Ticket 属性分析”面板，字段从历史成员实际属性自动发现，
因此 score、Level/playerLevel、region 等自定义属性无需写死到客户端；同名不同类型分开列出。
不包含仍在等待队列中的 Ticket，也不将当前 Contract 声明冒充历史成员数据。

- int64、uint64s：按成员值/列表元素取样，显示有效样本数、均值、最小值、最大值、P95；
  可选择每局均值、P95、最小值、最大值或样本数生成图表。原数值分析选择器也加入 Ticket 属性，
  可沿用折线图、Rule/Placement 聚合和详细方差/分位数统计。
- strings：按包含该类别的成员数生成柱状图。同一成员内重复类别只计一次，跨类别分别计数；
  占比以有该属性的成员为分母，合计可超过 100%。空列表单独计数；空字符串是有效类别，
  逗号不会拆分值。类别使用带引号的文本标签，以区别空字符串及特殊字符。
- 分别显示有属性、缺失属性、空列表、不可用成员快照和被排除的非精确数值；缺失不补 0，
  超出 JavaScript 安全整数范围的十进制字符串不强转为浮点数。
- PNG/SVG/CSV 复用相同图表数据。分类 CSV 导出类别成员数、Match IDs 和范围元数据；
  百分比在分类表格显示。当前历史不含成员快照时显示无数据提示，不从实时队列拼接旧比赛。
