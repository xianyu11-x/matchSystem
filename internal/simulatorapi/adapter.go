...
func generatorSpec(request CustomTicketsRequest) simulator.BatchGeneratorSpec {
	...
	for name, g := range request.AttributeGenerators {
		spec.AttributeGenerators[name] = simulator.AttributeGenerator{Type: g.Type, Source: g.Source, Ref: g.Ref, Values: g.Values, Set: g.Set, Min: g.Min, Max: g.Max, Count: g.Count, Replacement: g.Replacement, Distribution: g.Distribution, Mean: g.Mean, StdDev: g.StdDev}
	}
